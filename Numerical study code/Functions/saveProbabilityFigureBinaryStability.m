function saveProbabilityFigureBinaryStability(iteration, sampledVals, nominalVals, testedPoints, param, options)
	arguments
		iteration
		sampledVals
		nominalVals
		testedPoints
		param
		options.MakeLegend = false
        options.PlotNominal = false
	end
    ...
    for aeInd = 1:length(param.RadialWidth)
        ...
        view(2)
        shading interp
        if ~isempty(nominalVals) && options.PlotNominal
        ...
        for i = 1:length(testedPoints)
            ...
            if isfield(testedPoints, 'Broken') && testedPoints(i).Broken == 1
